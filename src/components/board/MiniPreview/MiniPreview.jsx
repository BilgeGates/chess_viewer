...
const MiniPreview = memo(function MiniPreview({
  ...
  return <div className="relative w-full h-full" style={{
    aspectRatio: '1 / 1'
        ...
        <canvas ref={canvasRef} className={`w-full h-full transition-opacity duration-200 ${isLoading ? 'opacity-0' : 'opacity-100'}`} style={{
      imageRendering: 'auto',
      display: 'block',
      borderRadius: '0'
    }} />

        {isLoading && <div className="absolute inset-0 flex items-center justify-center bg-surface/60">
            <div className="w-5 h-5 border-2 border-accent border-t-transparent rounded-full animate-spin"></div>
          </div>}

        {(hasError || error) && !isLoading && <div className="absolute inset-0 flex items-center justify-center bg-error/10 border border-error/30">
  ...
  return prevProps.fen === nextProps.fen && prevProps.lightSquare === nextProps.lightSquare && prevProps.darkSquare === nextProps.darkSquare && prevProps.pieceStyle === nextProps.pieceStyle && prevProps.size === nextProps.size;
});
