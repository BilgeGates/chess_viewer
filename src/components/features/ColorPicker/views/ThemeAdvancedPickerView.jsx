 ...
 * @returns {JSX.Element}
 */
const ThemeAdvancedPickerView = memo(function ThemeAdvancedPickerView({
  activeSquare,
  setActiveSquare,
  lightSquare,
  setLightSquare,
  ...
  const handleCanvasClick = useCallback(e => {
    ...
    if (activeSquare === 'light') setLightSquare(newColor);else setDarkSquare(newColor);
          ...
          <div className="flex flex-col gap-3 min-h-0 overflow-y-auto pr-2">
            <div className="grid grid-cols-2 gap-2 flex-shrink-0">
              <div className="flex items-center gap-2 p-2.5 bg-surface rounded-lg border border-border">
                <div className="w-10 h-10 border border-border shadow-sm flex-shrink-0 rounded" style={{
            ...
            <div className="flex-shrink-0">
              ...
              <div className="flex gap-2 mb-3">
                {['light', 'dark'].map(sq => <button key={sq} onClick={() => setActiveSquare(sq)} className={`flex-1 py-2 text-xs font-semibold rounded-lg transition-all ${activeSquare === sq ? 'bg-accent text-bg shadow-md' : 'bg-surface-elevated text-text-secondary hover:bg-surface-hover border border-border'}`}>
                    {sq.charAt(0).toUpperCase() + sq.slice(1)}
                  </button>)}
              </div>

              <div className="bg-surface rounded-xl border border-border p-3 mb-3">
                <canvas ref={canvasRef} width={180} height={280} onClick={handleCanvasClick} className="w-full rounded-lg cursor-crosshair border border-border shadow-inner" />
              </div>
...
}, (prev, next) => prev.activeSquare === next.activeSquare && prev.lightSquare === next.lightSquare && prev.darkSquare === next.darkSquare);
